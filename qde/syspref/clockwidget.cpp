    ...
    : QWidget(parent)
{
    ...
    m_digitFont.setPointSize(16);
    m_dateFont.setPointSize(18);
...
void WallClock::paintBackground(QPainter & painter)
{
    ...
    QColor ramka(palette().highlight().color().darker(120)); // g³êboki czerwony
...
void WallClock::setShowCurrentDateTime(bool showCurrentDateTime)
{
  ...
  if (m_showCurrentDateTime) {
	emit minutePassed(QDateTime::currentDateTime().time());
    m_timer->start(); 
  }	
  else {
    m_timer->stop();
...
void WallClock::updateTime()
{
  m_dateTime = QDateTime::currentDateTime(); 
  if (seconds == -1) {
	seconds = m_dateTime.time().second();
  }
  if (isVisible()) {
    update();
